...
Capa de **definiciones genéricas** compartidas entre features. Aquí viven las formas de los datos, no la lógica de cómo se consumen.

### Qué va en `src/data/`

- **Tipos e interfaces compartidos**: formas de datos (`Video`, `Channel`, etc.) que más de un feature necesita.
...
- **Tipos que solo usa un feature**: si un tipo no se importa desde más de un feature, vive en el mock o el slice del feature, no en `data/`.

### Reglas

- `data/` es la fuente de **formas y materia prima**, no de lógica de acceso.
- Si un generador empieza a recibir parámetros de dominio específico (filtros, queries), debe moverse al mock del feature.
- No duplicar tipos: si `data/` ya exporta una interface, el mock del feature la importa, no la redefine.
...
- **Tipos propios del feature** que no se comparten con otros features (`MockUser`, interfaces locales).
...
| ¿Es un tipo o interface compartido entre features?  | `src/data/`                               |
...
application/*/components → application/*/fragments, components/fragments, components/core, components/ui, hooks, zustand
application/*/fragments → components/ui, hooks
components/fragments → components/ui, hooks
components/core → components/fragments, components/ui, hooks
...
| `Data`       | Tipos, interfaces y generadores genéricos (`src/data/`)                   |
