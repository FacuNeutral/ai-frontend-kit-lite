...
import { PAGE_DOCS, type PageDoc, type DetailTab } from "../pages-explorer.mock";
...
export default function DebugDetailPanel() {
  ...
  const pageDoc = useMemo<PageDoc | undefined>(() => {
    const clean = activeRoute.split("?")[0].replace(/\/vid-.*$/, "/:id").replace(/\/[^/]+$/, (m) => m);
    return PAGE_DOCS.find((d) => {
      if (d.route === activeRoute) return true;
      if (d.route === clean) return true;
      if (d.route === "/watch/:id" && activeRoute.startsWith("/watch/")) return true;
      if (d.route === "/search" && activeRoute.startsWith("/search")) return true;
      if (d.route === "*" && activeRoute === "/this-does-not-exist") return true;
      return false;
    });
  }, [activeRoute]);
  ...
  return (
    <div className="w-80 shrink-0 border-l border-debug-border dark:border-debug-border-dark bg-debug-surface dark:bg-debug-surface-dark overflow-y-auto">
      {/* ==========================================
          Tab Bar
         ========================================== */}
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`
              ...
              ${
                detailTab === tab.key
                  ? "border-debug-accent text-debug-accent"
        ...
        <div>
          ...
          <div className="space-y-1">
            {doc.stores.map((s) => (
              <div
                key={s}
                className="text-sm font-mono px-2 py-1.5 rounded-(--radius-debug-tab) bg-debug-surface-overlay dark:bg-debug-surface-overlay-dark text-debug-accent"
              >
                {s}
              </div>
            ))}
          ...
          <ul className="space-y-2">
            {doc.processes.map((p, i) => (
              <li key={i} className="flex gap-2 text-sm text-debug-text-muted dark:text-debug-text-muted-dark">
                <span className="text-debug-accent font-semibold shrink-0">{i + 1}.</span>
      ...
      <div>
        ...
        <div className="space-y-3">
          {doc.sections.map((section, i) => (
            <div
              key={i}
              className="relative pl-4 border-l-2 border-debug-accent/30"
            >
              <div className="absolute -left-1.25 top-1 w-2 h-2 rounded-full bg-debug-accent" />
              <div className="flex items-center gap-1.5 mb-1">
                <Box size={13} className="text-debug-accent shrink-0" />
      ...
      <div>
        ...
        <div className="bg-debug-surface-overlay dark:bg-debug-surface-overlay-dark rounded-(--radius-debug-panel) p-3 font-mono text-[11px] text-debug-text dark:text-debug-text-dark">
          <div className="text-debug-text-muted dark:text-debug-text-muted-dark">
            {"<"}<span className="text-debug-accent">{doc.layout === "MainLayout" ? "MainLayout" : "Page"}</span>{">"}
          </div>
          ...
          <div className={doc.layout === "MainLayout" ? "pl-6" : "pl-3"}>
            <div className="text-debug-text-muted dark:text-debug-text-muted-dark">
              {"<"}<span className="text-debug-accent font-semibold">{doc.title}Page</span>{">"}
            </div>
...
function SectionTitle({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex items-center gap-1.5 mb-2">
      <span className="text-debug-accent">{icon}</span>
...
function ParamRow({ label, example, description }: { label: string; example: string; description: string }) {
  return (
    <div className="flex items-start gap-2 text-sm">
      <code className="shrink-0 px-1.5 py-0.5 rounded-(--radius-debug-tab) bg-debug-surface-overlay dark:bg-debug-surface-overlay-dark text-debug-accent font-mono text-xs">
        {label}
      </code>
      <div>
